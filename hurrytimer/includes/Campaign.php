...
class Campaign {
    ...
    public $recurringUnselectedDaysAction = 'hide';
        ...
        try {
            ...
            while ($period->valid()) {
                ...
                if ($date->greaterThan($now)) {
                    ...
                    if ($this->can_recur_on($date)) {
                        break;
                    }

                    if ($this->should_skip_day($date)) {

                        if ($this->recurringUnselectedDaysAction === 'hide' || $this->isWeeklyRecurring()) {
                            return false;
                        }
    ...
    function should_skip_day($date) {
        $recur_days = array_map('intval', $this->recurringDays);
        return !in_array($date->format('w'), $recur_days);
    }
