...
class AJAX extends Base {
	...
	public function analyze() {
		...
		if( ! wp_verify_nonce( $_POST['_wpnonce'], 'cxis-optimize' ) ) {
			$response['message'] = __( 'Unauthorized', 'image-sizes' );
			wp_send_json( $response );
		}

		if( ! isset( $_POST['image-type'] ) || count( $_POST['image-type'] ) <= 0 ) {
			$response['html'] = '<p><span class="dashicons dashicons-dismiss cx-icon cx-error"></span>' . __( 'Please select some image types first!', 'image-sizes-pro' ) . '</p>';
			wp_send_json( $response );
		}
		...
		if( ! $analyze_only ) {
			$response = [
				'status'	=> 1,
				'message'	=> __( 'Success', 'image-sizes-pro' ),
				'images'	=> $images,
				'size'		=> $total_size,
		...
		if( count( $queried_images ) <= 0 ) {
			$response['html'] = '<p><span class="dashicons dashicons-dismiss cx-icon cx-error"></span>' . __( 'No images have been found!', 'image-sizes-pro' ) . '</p>';
			wp_send_json( $response );
		}
