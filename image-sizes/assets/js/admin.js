jQuery(function($){
	...
	$('#cxis-regen-thumbs').click(function(e){
		...
		$('#cxis-regen-thumbs').after('<div class="image-sizes-progress-panel"><div class="image-sizes-progress-content" style="width:0%"><span>0%</span></div></div></div>');
	});
	...
	// show pro demo
	$('.cxis-action').click(function(e){
		e.preventDefault()
		$('#cxis-optimize').attr('disabled',true)
		$('#cxis-pro-message').html('')

		var $form = $('#cx-free-optimize #cx-optimize');
		var $button = $(this)
		var $formData = $form.serializeArray();
		var $operation = $(this).attr('name') == 'analyze' ? 'analyze' : 'optimize';
		$button.val( $operation == 'optimize' ? CXIS.optimizing : CXIS.analyzing ).attr('disabled', true);
		...
		$.ajax({
			...
			success: function(resp) {
				if( resp.status == 1 ) {
					$('#cxis-optimize').attr('disabled',false)
				}
				$button.val( $operation == 'optimize' ? CXIS.optimize : CXIS.analyze ).attr('disabled', false);
				console.log(resp);
				$('#cxis-pro-message').html(resp.html)
			},
			error: function(err) {
				$button.val( $operation == 'optimize' ? CXIS.optimize : CXIS.analyze ).attr('disabled', false);
				console.log(err);
